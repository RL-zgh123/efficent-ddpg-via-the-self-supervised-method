...
class AssistNet(object):
    ...
    def get_phi(self, s):
        return sess.run(self.phi_s, {self.s: s})
...
class Actor(object):
    ...
    def _build_net(self, phi_s, scope, trainable):
...
class Critic(object):
    ...
    def _build_net(self, phi_s, a, scope, trainable):
        with tf.variable_scope(scope):
...
class Memory(object):
    ...
    def store_transition(self, s, phi_s, a, r, s_, phi_s_):
        transition = np.hstack((s, phi_s, a, [r], s_, phi_s_))
